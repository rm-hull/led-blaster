/*
 * led-blaster.c Driving RGB LEDs with PWM/BCM using DMA on the Raspberry Pi
 * Copyright (c) 2017 Richard Hull <rm_hull@yahoo.co.uk>
 *
...
#define DEVFILE         "/dev/led-blaster"
#define DEVFILE_MBOX    "/dev/led-blaster-mbox"
...
map_peripheral(uint32_t base, uint32_t len)
{
    ...
    if (fd < 0)
        fatal("led-blaster: Failed to open /dev/mem: %m\n");
    vaddr = mmap(NULL, len, PROT_READ|PROT_WRITE, MAP_SHARED, fd, base);
    if (vaddr == MAP_FAILED)
        fatal("led-blaster: Failed to map peripheral at 0x%08x: %m\n", base);
    close(fd);
...
// Set the pin to a pin2gpio element so led-blaster can write to it,
...
// command to the /dev/led-blaster file. We make sure to compact the pin2gpio array
...
set_pin(int pin, float width)
{
  if (is_known_pin(pin)){
    ...
    fprintf(stderr, "GPIO %d is not enabled for led-blaster\n", pin);
  }
}
...
go_go_go(void)
{
    ...
    if ((fp = fopen(DEVFILE, "r+")) == NULL)
        fatal("led-blaster: Failed to open %s: %m\n", DEVFILE);
...
main(int argc, char **argv)
{
    ...
    if ((unsigned long)mbox.virt_addr & (PAGE_SIZE-1))
        fatal("led-blaster: Virtual address is not page aligned\n");
    ...
    if (mkfifo(DEVFILE, 0666) < 0)
        fatal("led-blaster: Failed to create %s: %m\n", DEVFILE);
    if (chmod(DEVFILE, 0666) < 0)
        fatal("led-blaster: Failed to set permissions on %s: %m\n", DEVFILE);

    printf("Initialised, ");
    if (daemonize) {
        if (daemon(0,1) < 0)
            fatal("led-blaster: Failed to daemonize process: %m\n");
        else
            printf("Daemonized, ");
    }
    printf("Reading %s.\n", DEVFILE);
